...
beta = 3.0; % Heaviside regularization constant
...
%x0 = random('unif', 0.0, 1.0, xn*xm, 1); % random initial distribution for testing sensitivities
  ...
  F_ext = sparse(2*(Ex+1)*(Ey+1)-Ey,1,1,2*(Ey+1)*(Ex+1),1);
  fixeddofs=[1 2];
  for i=1:Ex
      fixeddofs=[fixeddofs 1+i*(Ey+1)*2 2+i*(Ey+1)*2];
  end;
...
xval=reshape(xval,xn,xm);
for i=1:xm
    tmp=reshape(xval(:,i),Ey,Ex);
    tmp(1:Ey/2,Ex/2+1:Ex)=1;
    xval(:,i)=reshape(tmp,xn,1);
end;
xval=reshape(xval,xn*xm,1);


% Solve linear FEM problem; evaluate objective and sensitivities
lambda=0.01;
[C, dC] = comp_fast(xval, F_ext, constraints,lambda);
